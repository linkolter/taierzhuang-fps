...
export class Navigation {
  width=177; depth=85; step=1; walkable: boolean[]=[];
  ...
  find(start:Vector3,end:Vector3){
    ...
    let iterations=0;
    while(open.size&&iterations++<16000){let current=-1,best=Infinity;for(const i of open){const f=cost.get(i)!+heuristic(i);if(f<best){best=f;current=i;}}if(current===e){const path:Vector3[]=[];let n=e;while(n!==s){path.unshift(this.point(n));n=came.get(n)!;}path.push(end.clone());return this.simplify(start,path);}

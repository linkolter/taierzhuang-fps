...
export class World {
  ...
  box(name: string, x: number, y: number, z: number, w: number, h: number, d: number, mat: StandardMaterial, solid = true) {
    ...
    if (solid) { this.obstacles.push({ x, z, w, d, bottom: y - h / 2, top: y + h / 2 }); this.solids.push(m); this.collisionIndex?.add(this.obstacles.length-1); } return m;
  }
  buildTraining() {
  ...
  buildVillage() { this.village = true; buildVillage(this); this.collisionIndex=new ObstacleIndex(this.obstacles); this.navigation = new Navigation(this);this.tactical=new TacticalRouteGraph(this);this.coverPoints=buildCoverPoints(this); this.optimizeStatic(); }

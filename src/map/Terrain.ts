...
export class Terrain {
 height=(x:number,z:number)=>Math.max(interpolate(HIGH_PROFILE,x)*clamp((z-14)/10,0,1),Math.min(clamp((13-Math.abs(x))/7,0,1),clamp((11-Math.abs(z))/5,0,1)))-(1.3+.4*clamp((x+70)/140,0,1))*clamp((-z-14)/12,0,1);
 build(w:World){
  ...
  for(let x0=-90;x0<90;x0+=30)for(let z0=-45;z0<45;z0+=15){
   ...
   const mesh=new Mesh(`terrain-${x0}-${z0}`,w.scene);data.applyToMesh(mesh);mesh.material=w.material('earth','#a89671');mesh.metadata={solid:true,terrain:true,staticChunk:true};mesh.isPickable=true;mesh.receiveShadows=true;chunks.push(mesh);
  }
  return chunks;
 }
}

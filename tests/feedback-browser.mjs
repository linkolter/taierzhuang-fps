...
try{
 await page.goto('http://127.0.0.1:5173');await page.waitForFunction(()=>window.__game?.scene.isReady());await page.locator('#start').click();
 const checks=await page.evaluate(async()=>{
  ...
  const front=g.scene.getMeshByName('front-sight'),rear=g.scene.getMeshByName('rear-sight-left'),V=g.player.position.constructor;
  const projected=V.Project(new V(0,.02,0),front.getWorldMatrix(),g.scene.getTransformMatrix(),g.player.camera.viewport.toGlobal(1440,900));
  ...
  const woodShared=g.world.materials.get('wood').diffuseTexture===g.world.materials.get('rifle-stock').diffuseTexture;
  ...
  return {initial,final:sizes(),firstFlash:window.__firstFlash,boltLock:window.__boltLock,rifleCount,boltSounds,reload,ads,materialKinds,woodShared,shellCapacity,shellsRecycled,maxVoices,audioCleaned,covers:g.world.coverPoints.length};
 });
